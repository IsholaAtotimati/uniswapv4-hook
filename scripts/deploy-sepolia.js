...
async function main() {
  const { SEPOLIA_RPC_URL, DEPLOYER_PRIVATE_KEY, POOL_MANAGER_ADDRESS } = process.env;
  if (!SEPOLIA_RPC_URL || !DEPLOYER_PRIVATE_KEY || !POOL_MANAGER_ADDRESS) {
    console.error('Missing env vars. See .env.example');
    process.exit(1);
  }

  // Basic validation to catch placeholder values like "0x..." or
  // "0xPOOL_MANAGER_ADDRESS_ON_SEPOLIA" which will cause ethers to fail.
  if (
